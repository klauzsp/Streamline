...
try {
  ...
  assert.ok(
    (await page.locator(".gd-selected-reference").innerText()).includes(
      alternative.label,
    ),
  );
  await page.route("**/api/migrations/*", async (route) => {
    const req = route.request();
    if (req.method() !== "POST" || req.postDataJSON().action !== "investigate")
    ...
    state.migration.suggestions[input.mappingId] = {
      candidates: [],
      explanation: "TEST: insufficient evidence",
      advice: {
        recommendation: "Do not approve yet.",
        evidence: "TEST: sampled descriptions",
        uncertainty: "Only a sample was reviewed.",
        nextAction: "Ask the previous administrator to identify the holding.",
      },
  ...
  await page
    ...
    .click();
  await page
    .getByText("Do not approve yet.", { exact: true })
    .waitFor();
  assert.equal(await page.locator(".gd-advice dt").count(), 4);
  ...
  console.log(
    "Review selection passed: alternative copy agrees, confirmation resets, abstention clears target and blocks approval.",
  );
} finally {
  await browser.close();
}
